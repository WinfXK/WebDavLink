...
public class Yaml {
	...
	/**
	 * Create Yaml instance. It is safe to create a few instances and use them in
	 * different Threads.
	 */
	public Yaml() {
	 ...
	 *
	 * <p>
	 * This method is similar to <code>Yaml.dump(data)</code> except that the root
	 * tag for the whole document is replaced with the given tag. This has two main
	 * uses.
	 * </p>
	 *
	 * <p>
	 ...
	 * will appear as <code>!!map</code>, or blank (implicit !!map).
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @param data      Java object to be serialized to YAML
	 * @param rootTag   the tag for the whole YAML document. The tag should be
	 *                  Tag.MAP for a JavaBean to make the tag disappear (to use
	 *                  implicit tag !!map). If <code>null</code> is provided then
	 *                  the standard tag with the full class name is used.
	 * @param flowStyle flow style for the whole document. See Chapter 10.
	 *                  Collection Styles http://yaml.org/spec/1.1/#id930798. If
	 ...
	 * <p>
	 * This method is similar to <code>Yaml.dump(data)</code> except that the root
	 * tag for the whole document is replaced with <code>Tag.MAP</code> tag
	 * (implicit !!map).
	 * </p>
	 * <p>
	...
	/**
	 * Parse the only YAML document in a String and produce the corresponding Java
	 * object. (Because the encoding in known BOM is not respected.)
	 *
	...
	/**
	 * Parse the only YAML document in a stream and produce the corresponding Java
	 * object.
	 *
	 * @param io  data to load from (BOM is respected to detect encoding and removed
	 *            from the data)
	 * @param <T> the class of the instance to be created
	 * @return parsed object
	 */
	 ...
	 *
	 * @param <T>   Class is defined by the second argument
	 * @param input data to load from (BOM is respected to detect encoding and
	 ...
	 * (Because the encoding in known BOM is not respected.) The documents are
	 ...
	 *
	 * @param yaml YAML data to load from (BOM is respected to detect encoding and
	 *             removed from the data)
