...
public final class Emitter implements Emitable {
	...
	private boolean checkSimpleKey() {
		...
		return length < maxSimpleKeyLength && (event instanceof AliasEvent
				|| (event instanceof ScalarEvent && !analysis.empty && !analysis.multiline) || checkEmptySequence()
				|| checkEmptyMapping());
	}
	...
	private void processTag() throws IOException {
		...
		if (event instanceof ScalarEvent) {
			...
			if ((!canonical || tag == null) && ((style == null && ev.getImplicit().canOmitTagInPlainScalar())
	...
	private ScalarAnalysis analyzeScalar(String scalar) {
		...
		return new ScalarAnalysis(scalar, false, lineBreaks, allowFlowPlain, allowBlockPlain, allowSingleQuoted,
				allowBlock);
	}
