...
public class Constructor extends SafeConstructor {
	...
	/**
	 * Create Constructor for a class which does not have to be in the classpath or
	 * for a definition from a Spring ApplicationContext.
	 *
		...
		protected Object constructJavaBean2ndStep(MappingNode node, Object object) {
			...
			for (NodeTuple tuple : nodeValue) {
				...
				try {
					...
					Object value = (memberDescription != null) ? newInstance(memberDescription, key, valueNode)
					...
					throw new ConstructorException("Cannot create property=" + key + " for JavaBean=" + object,
	...
	/**
	 * Construct an instance when the runtime class is not known but a global tag
	 * with a class name is defined. It delegates the construction to the
	 * appropriate constructor based on the node kind (scalar, sequence, mapping)
	 */
	protected class ConstructYamlObject implements Construct {
		...
		public void construct2ndStep(Node node, Object object) {
			try {
				...
				throw new ConstructorException(null, null, "Can't construct a second step for a java object for "
	...
	protected class ConstructScalar extends AbstractConstruct {
		...
		public Object construct(Node nnode) {
			...
			if (type.isPrimitive() || type == String.class || Number.class.isAssignableFrom(type)
					...
					|| type == BigInteger.class || type == BigDecimal.class || Enum.class.isAssignableFrom(type)
				...
				if (javaConstructor == null) {
					try {
						...
						throw new YAMLException(
								"No single argument constructor found for " + type + " : " + ie.getMessage());
					}
		...
		private Object constructStandardJavaInstance(@SuppressWarnings("rawtypes") Class type, ScalarNode node) {
			...
			if (type == String.class) {
				...
				if (type == BigDecimal.class) {
					...
					|| type == BigInteger.class || type == Byte.TYPE || type == Short.TYPE || type == Integer.TYPE
					|| type == Long.TYPE) {
	...
	/**
	 * Construct sequence (List, Array, or immutable object) when the runtime class
	 * is known.
	 */
	protected class ConstructSequence implements Construct {
		...
		public Object construct(Node node) {
			...
			if (Set.class.isAssignableFrom(node.getType())) {
				...
				throw new YAMLException("No suitable constructor with " + String.valueOf(snode.getValue().size())
