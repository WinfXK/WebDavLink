...
import cn.winfxk.android.myclouds.BuildConfig;
...
public class DialProgress extends View {
			...
			try {
				...
				if (gradientColors.length == 0) {
					...
					if (Build.VERSION.SDK_INT >= 23) {
						color = getResources().getColor(gradientArcColors, context.getTheme());
					} else
						color = getResources().getColor(gradientArcColors);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		updateArcPaint();
		Log.d(TAG, "onMeasure: 控件大小 = " + "(" + getMeasuredWidth() + ", " + getMeasuredHeight() + ")" + ";圆心坐标 = "
				+ mCenterPoint.toString() + ";圆半径 = " + mRadius + ";圆的外接矩形 = " + mRectF.toString());
	}
	...
	private void drawDial(Canvas canvas) {
		...
		for (int i = 0; i <= total; i++) {
			canvas.drawLine(mCenterPoint.x + mRadius, mCenterPoint.y, mCenterPoint.x + mRadius + mArcWidth,
					mCenterPoint.y, mDialPaint);
		...
		mAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				if (BuildConfig.DEBUG)
					Log.d(TAG, "onAnimationUpdate: percent = " + mPercent + ";currentAngle = "
							+ (mSweepAngle * mPercent) + ";value = " + mValue);
				invalidate();
			}
		});
		mAnimator.start();
